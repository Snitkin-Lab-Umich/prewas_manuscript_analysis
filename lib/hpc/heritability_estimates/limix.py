...
phen_paths = glob.glob('data/*_prewas_output_with_ancestral_reconstruction_given_tree_phenotype.csv')
phen_paths.sort()

shared_ref_multi_paths = glob.glob('data/*shared_ref_multi.csv')
shared_ref_multi_paths.sort()
shared_maj_multi_paths = glob.glob('data/*shared_maj_multi.csv')
shared_maj_multi_paths.sort()
shared_anc_multi_paths = glob.glob('data/*shared_anc_multi.csv')
shared_anc_multi_paths.sort()

shared_ref_nomulti_paths = glob.glob('data/*shared_ref_nomulti.csv')
shared_ref_nomulti_paths.sort()
shared_maj_nomulti_paths = glob.glob('data/*shared_maj_nomulti.csv')
shared_maj_nomulti_paths.sort()
shared_anc_nomulti_paths = glob.glob('data/*shared_anc_nomulti.csv')
shared_anc_nomulti_paths.sort()

datasets = [re.sub('data/|_prewas.*csv','',x) for x in phen_paths]
basename = [re.sub('data/|_phenotype.*csv','',x) for x in phen_paths]
...
with open('data/her_ests_shared.csv', "w") as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)
