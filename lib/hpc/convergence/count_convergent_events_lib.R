...
count_convergent_events <- function(snpmat, tree) {
  ancestral_recon <- rep(list(0), nrow(snpmat))
  transition_mat <- matrix(0, nrow = nrow(snpmat), ncol = 1)
  ...
  for (i in 1:nrow(snpmat)) {
    ...
    transition_direction <- parent_node <- child_node <- integer(ape::Nedge(tree))
    ...
    for (j in 1:Nedge(tree)) {
      ...
      if (tree$edge[j, younger] <= ape::Ntip(tree)) {
        ...
        child_node[j]  <- unlist(current_recon[tree$edge[j, younger] - ape::Ntip(tree)])
      }
...
plot_num_convergence_histogram <- function(biallelic_trans_mat,
  ...
  legend("topright", c("Biallelic", "Multiallelic"), fill = c(bi_color, multi_color))
  dev.off()
}
...
save_results <- function(project_name, 
  ...
  write.csv(biallelic_trans_mat, 
            file = paste0("../data/", project_name, "_biallelic_transition_matrix.csv"))
  write.csv(multiallelic_trans_mat, 
            file = paste0("../data/", project_name, "_multiallelic_transition_matrix.csv"))
}