...
table1 <- table1 %>%
  select(Dataset, NumIsolates, NumMultiallelicSite, MeanSNPDist, NumBiallelicOverlappingGene)
colnames(table1) <- c("Name", "Samples (Count)", "Multiallelic Sites (Count)", "Mean SNP Distance (BP)", "SNPs in overlapping genes (Count)")
